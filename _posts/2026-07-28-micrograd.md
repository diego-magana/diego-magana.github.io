...
I rebuilt micrograd — Karpathy's scalar autograd engine — from scratch, following his Zero to Hero lecture: a `Value` class that builds a computation graph through Python operator overloading, then runs reverse-mode automatic differentiation over it. Scalar-valued means every neuron gets decomposed into its individual adds and multiplies — hopeless for performance, ideal for seeing the mechanism, since each node in the graph carries exactly one number and one gradient. On top of the engine sits a small neural net library (`Neuron`, `Layer`, `MLP`), a test suite that checks every gradient against PyTorch, and a demo that trains an MLP on three interleaved spirals — a 2D, three-class problem I generate directly, without sklearn. The packaging, the gradient tests, and the spiral experiment are mine. Code: [github.com/diego-magana/micrograd](https://github.com/diego-magana/micrograd).
...
That's the real economy of the design: derived operations don't need derived gradients. Six local rules plus the chain rule differentiate any composition built from them — including compositions the engine has never heard of.
...
The third is easy to miss because it hides in an ordinary line of Python — and in this engine it hides inside the softmax itself. Building the denominator is `sum(counts)`, and `sum()` starts from the integer `0`, so the first operation is `0 + Value` — `int.__add__` fails, and Python falls back to `Value.__radd__`. Without the reflected operators, the engine works in every expression I write by hand and breaks the first time a standard-library idiom touches it. I stopped thinking of `__radd__` and friends as polish; they're what makes the type behave like a number.

Since none of this announces itself, the tests are the correctness argument, not hygiene. I verified every operator — and the full softmax+NLL composition, diamond included — against PyTorch's autograd. The assertion is 1e-5 absolute, which on the tangled test case works out much tighter than it sounds: the gradients there reach $10^5$, so that's agreement to eleven significant figures. Two independent implementations matching that closely means the graph construction and the chain-rule application are both right. One detail: I cast the PyTorch reference tensors to `float64`, because the default `float32` floors the comparison around 1e-7 and fails tests against Python floats for no real reason.

The claim that these bugs are silent is itself testable, so the suite tests it. `test_accumulation_is_not_assignment` swaps the `+=` for `=`, trains a small MLP on thirty points, and asserts the loss still falls — 46.3 down to under 12, on a network where every gradient is wrong.

It's worse than silent. Run the broken version twice and you get different answers — 8.77, 9.55, 11.42 from identical seeds — because `backward()` orders the graph by iterating a `set` of nodes hashed by identity, so traversal order shifts with memory layout between runs. With `+=` that's invisible: every path contributes and addition commutes. With `=` the last writer wins, and which node writes last is an accident. Which means the second bug here isn't independent of the first — accumulation is what makes the traversal order stop mattering.
...
`scripts/loss_floor.py` in the repo rebuilds that network so this is checkable rather than remembered. It has to stack the layers by hand, because `MLP` won't produce a tanh head any more — that's the fix, below. A fresh run lands at 32.20 against the same 31.33: the floor reproduces, the exact historical number doesn't.
...
Except they aren't two events. Invert the number: a summed loss of 31.84 over 100 samples is a mean BCE of 0.3184, so the mean predicted probability is $e^{-0.3184} = 0.727$, which puts the tanh output at 0.981 — and there the local gradient $1 - \tanh^2$ is 0.0376. The output neuron was pinned against its ceiling *and* passing back four percent of the gradient it received. The confidence cap and the vanishing gradient are the same saturation, read from two ends.
...
The fix is the one every framework quietly uses: a linear output layer that emits unbounded logits and leaves the squashing to the loss function. The current demo in the repo applies it directly — `MLP(2, [16, 16, 3])`, 371 parameters, softmax+NLL on three-class spirals with a linear output head and a real train/test split (train 0.99 / test 0.87).

The spirals are a harder problem than the moons, and I wanted that measured rather than asserted, so I ran a linear baseline: the same softmax head with no hidden layers, nine parameters. It converges to a held-out NLL of 1.14 on this split, against the $\ln 3 \approx 1.10$ of guessing uniformly. It isn't undertrained — the training loss stops moving at step 80 and hasn't moved by step 3,000, settling at finite weights instead of running them off toward infinity the way a linear model does when the classes really are separable. It fits the training set a little (1.00) and none of that transfers.

Accuracy says the same thing less stably: 0.27 on 30 test points, with predictions falling [14, 8, 8] against true counts of 7/11/12 — it collapses onto one class rather than separating the arms. That's what a linear boundary does when the classes wrap around each other. It can't draw the curve, so it picks a half-plane and eats the loss. The MLP reaches 0.87 on that same split, and the hidden layers are the entire difference.

## Where this stops

The scalar design is a deliberate ceiling, not an oversight. One `Value` per number and a Python loop per forward pass make this $O(\text{samples} \times \text{params})$ with no vectorization — fine for a hundred 2D points, hopeless for anything real. The point was never speed. It was that you can read every gradient.

Two other boundaries worth naming. The test-below-train gap is small-data reality — a few dozen points per class with genuine overlap near the origin — rather than a bug I haven't found. And the softmax here is the plain form: at larger logit magnitudes it needs the log-sum-exp trick to stay numerically stable, which is the first thing makemore has to fix.

## What's next

The finished engine differentiates anything I can express as a composition of its scalar operations — softmax, NLL, BCE, and an arbitrary MLP are all just graphs it walks the same way. Backpropagation stopped being a black box for me at the point where I had written all eight closures and watched an independent implementation confirm every one of them.
